...
class Environment:
    ...
    def __init__(self, width, height, simulator: ISimulator):
    ...
    def add_base_station(self, base_stations: list):
        ...
        log(
            "Added {} base stations in the environment.".format(
                len(base_stations)
            )
        )
    ...
    def get_current_cell(self, drone):
        ...
        cell_index = TraversedCells.coord_to_cell(
            size_cell=self.simulator.grid_cell_size,
    ...
    def spawn_obstacles(self, orthogonal_obs=False):
        ...
        for i in range(self.simulator.n_obstacles):
            startx, starty = self.simulator.rnd_env.randint(
                0, self.width
            ), self.simulator.rnd_env.randint(0, self.height)
            length = self.simulator.rnd_env.randint(100, 300)
            angle = (
                self.simulator.rnd_env.randint(0, 359)
                if not orthogonal_obs
                else self.simulator.rnd_env.choice([0, 90, 180, 270], 1)[0]
            )

            endx, endy = startx + length * np.cos(
                np.radians(angle)
            ), starty + length * np.sin(np.radians(angle))
        ...
        for i, coords in enumerate(target_coord):
            self.targets.append(
                Target(
                    i,
                    coords,
                    self.simulator.simulation_duration_sec(),
                    self.simulator,
                )
            )
    ...
    def detect_collision(self, drone):
        ...
        if len(self.obstacles) > 0:
            ...
            critic_obstacles = np.array(self.obstacles)[
                distance_obstacles <= distance_travelled
            ]  # obstacles that could be crossed in a time step

            for critical_segment in critic_obstacles:
                p1, p2 = (critical_segment[0], critical_segment[1]), (
                    critical_segment[2],
                    critical_segment[3],
                )
                ...
                if (
                    is_segments_intersect(p1, p2, p3, p4)
                    or distance_point_segment(p1, p2, p4) < 1
                ):
