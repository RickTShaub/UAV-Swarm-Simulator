...
class Drone(SimulatedEntity, AntennaEquippedDevice):
    ...
    def __init__(
        ...
        self.com_range, self.sensing_range, self.radar_range = (
            com_range,
            sensing_range,
            radar_range,
        )
    ...
    def move(self):
        ...
        if not self.simulator.free_movement:
            ...
            self.angle = angle_between_three_points(
                self.next_target(), np.array(self.coords), horizontal_coo
            )
    ...
    def will_reach_target(self):
        ...
        return (
            self.speed * self.simulator.ts_duration_sec
            >= euclidean_distance(self.coords, self.next_target())
        )

    def increase_waypoint_counter(self):
        """Cyclic visit in the waypoints list."""
        self.current_waypoint_count = (
            self.current_waypoint_count + 1
            if self.current_waypoint_count < len(self.path) - 1
            else 0
        )
    ...
    def drop_expired_packets(self, ts):
        ...
        for packet in self.buffer:
            if packet.is_expired(ts):
                ...
                log(
                    "drone: {} - removed a packet id: {}".format(
                        str(self.identifier), str(packet.identifier)
                    )
                )
