...
class Target(SimulatedEntity):
    ...
    def age_of_information(self):
        return (
            self.simulator.cur_step - self.last_visit_ts
    ...
    def oldest(set_targets, cur_tar):
        ...
        max_aoi = np.argmax(
            [target.age_of_information() for target in set_targets]
        )
    ...
    def lowest_residual(set_targets, cur_tar):
        ...
        min_res = np.argmin(
            [target.residual_of_information() for target in set_targets]
        )
