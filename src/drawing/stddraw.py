...
from .color import VERY_LIGHT_GRAY
...
_DEFAULT_YMAX = 1.0
# Hack: fix later
_DEFAULT_CANVAS_SIZE = config.SimConfig().draw_size
...
def _show(simulator):
    ...
    pygame.display.flip()
    _checkForEvents(simulator)


def _showAndWaitForever(simulator):
    """
...
def _checkForEvents(simulator):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            simulator.detect_key_pressed(event.unicode)
        ...
        # the mouse position as of that press.
            ...
            _mousePos = event.pos
            # x,y = event.pos
            # print(x, y, x, 700-y, 1500/700*x, 1500/700*(700-y))
            simulator.detect_drone_click(event.pos)
        # ---------------------------------------------------------------
        # End added by Alan J. Broder
        # ---------------------------------------------------------------
