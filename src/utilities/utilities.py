...
class EventGenerator:
    ...
    def handle_events_generation(self, cur_step: int, drones: list):
        ...
        if (
            cur_step > 0
            and cur_step % self.simulator.event_generation_delay == 0
        ):  # if it's time to generate a new packet
            print(
                self.simulator.event_generation_delay,
                cur_step,
                cur_step % self.simulator.event_generation_delay,
            )
...
class PathToDepot:
    ...
    def __init__(self, x_position, simulator: ISimulator):
    ...
    def next_target(self, drone_pos):
        ...
        if (
            ...
            return (
                self.x_position,
                drone_pos[1],
            )  # the closest point to the channel
...
class TraversedCells:
    ...
    def cells_in_travel(size_cell, width_area, start, end):
        ...
        for cell in coords_index:
            ...
            ur = (
                cell[0] * size_cell + size_cell,
                cell[1] * size_cell + size_cell,
            )
    ...
    def intersect_segments(
        start1: tuple, end1: tuple, start2: tuple, end2: tuple
    ):
        if end1 == start2:
            return True
        if end2 == start1:
        ...
        b = np.asarray(start2) - np.asarray(
            end2
        )  # direction of line b, reversed
        ...
        for x in range(0, widht_area, size_cell):
            for y in range(0, height_area, size_cell):
                all_cells_and_centers.append(
                    (
                        TraversedCells.coord_to_cell(
                            size_cell, widht_area, x, y
                        ),
